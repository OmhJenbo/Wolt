...
def _(user_pk):
    try:
        ...
        db, cursor = x.db()
        cursor.execute("UPDATE users SET user_deleted_at = %s WHERE user_pk = %s", (user["user_deleted_at"], user["user_pk"]))
        ...
        return f"""
                <template mix-target='#trashcan-{user_pk}' mix-replace> 
...
def _(user_pk):
    try:
        ...
        try:
            ...
            toast = template("__toast.html", message="System Crashed")
        return f"""
                <template mix-target="#toast" mix-bottom>
                    {toast}
                </template>
